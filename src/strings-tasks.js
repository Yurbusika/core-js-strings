...
function countVowels(str) {
  ...
  const m = [...str.matchAll(/[aeiouyAEIOUY]/gi)];
  return m.length;
}
...
function isPalindrome(str) {
  const regular = str.replaceAll(/[\s,!?]/g, '').toLowerCase();
  const reversed = regular.split('').reverse().join('');
...
function findLongestWord(sentence) {
  const words = sentence.split(' ').sort((a, b) => {
